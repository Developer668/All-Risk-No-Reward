    ...
    <main className="legal-page">
      ...
      <article className="legal-document shell">
        ...
        {privacy ? <>
          ...
          <section><h2>AI proof checks</h2><p>Challenge loading, filtering, selection, and evidence validation are deterministic and do not use AI. Before a visual proof check, the app identifies the selected provider: Google Gemini, OpenRouter, or NVIDIA NIM. You may use the operator-configured provider or connect your own key in Settings. Personal keys are stored in this browser's session by default and are never synced or exported; remembering a key on this device is optional. In local mode your proof goes directly to the selected provider. In synced mode it is securely proxied through the authenticated InsForge edge function, which uses a personal key only for that request and does not store it. Images are compressed and stripped of embedded metadata in your browser. Only a proof hash, media type, size, and optional filename are retained by this app. Provider retention, training, availability, and pricing terms differ. Never include confidential information, names, faces, private replies, or identifying information.</p></section>
