    ...
    <main className="legal-page">
      ...
      <article className="legal-document shell">
        ...
        {privacy ? <>
          ...
          <section><h2>AI proof checks</h2><p>Challenge loading, filtering, selection, and evidence validation are deterministic and do not use AI. For a remote proof check, the app tells you that your proof note and required image or short video will be securely proxied through the InsForge edge function to the operator-configured Gemini, OpenRouter, or NVIDIA NIM provider only for visual interpretation. InsForge is only the secure proxy; it does not perform the AI assessment. Images are compressed and stripped of embedded metadata in your browser. Only a hash, media type, size, and optional filename are retained by this app. Provider retention, training, availability, and pricing terms differ by provider and account tier, so the operator must disclose the selected provider and review its current terms before launch. Never include confidential information, names, faces, private replies, or identifying information.</p></section>
