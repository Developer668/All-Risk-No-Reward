...
await page.getByLabel('Optional context').fill('I gave a specific compliment about how thoughtfully a classmate explained an idea. I felt nervous, then they smiled and thanked me.')
...
await recoveryPage.getByRole('button', { name: 'Preview sample data' }).click()
await recoveryPage.getByRole('button', { name: /Add privacy-safe proof/ }).click()
await recoveryPage.locator('input[type="file"]').setInputFiles('public/og.png')
await recoveryPage.getByLabel('Optional context').fill('I spoke briefly today.')
await recoveryPage.getByRole('button', { name: /Preview proof result/ }).click()
